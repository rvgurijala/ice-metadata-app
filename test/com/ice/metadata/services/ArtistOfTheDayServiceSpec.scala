...
import com.ice.metadata.persistence.{ArtistPersistence, TrackPersistence}
...
class ArtistOfTheDayServiceSpec extends PlaySpec with GuiceOneAppPerTest with Injecting {
  ...
  "ArtistOfTheDayService" should {
    ...
    "get artist of the day" in {
      ...
      val track = trackRepository.save(fakeTrack.toTrack())
      track.fold(e => (), res => res.title mustBe fakeTrack.title)

      val artistOfTheDayService = new ArtistOfTheDayService(artistService)
      artistOfTheDayService.getArtistOfTheDay().fold(e => (), res => res.name mustBe fakeArtist.name)
    }
